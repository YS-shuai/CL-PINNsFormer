...
for i in tqdm(range(1000)):
    def closure():
        ...
        #print('Loss Res: {:4f}, Loss_BC: {:4f}, Loss_IC: {:4f}'.format(loss_track[-1][0], loss_track[-1][1], loss_track[-1][2]))
        loss = loss_res + 10 * loss_bc + loss_ic
        #loss = loss_ic
        optim.zero_grad()
        loss.backward()
        return loss
...
if not os.path.exists('./results/convection/'):
    os.makedirs('./results/convection/')

torch.save(model.state_dict(), f'./results/convection/1dwave_{args.model}_{num_step}_{step_size}.pt')
