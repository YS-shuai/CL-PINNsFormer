...
if args.model == 'PINNsFormer' or args.model == 'CL_PINNsFormer' or args.model == 'PINNMamba':
...
elif args.model == 'PINNsFormer' or args.model == 'CL_PINNsFormer'or args.model == 'PINNMamba':
...
for i in tqdm(range(500)):
    def closure():
        ...
        if i % 20 == 0:
            print('Loss Res: {:4f}, Loss_BC: {:4f}, Loss_IC: {:4f}'.format(loss_track[-1][0], loss_track[-1][1],
                                                                           loss_track[-1][2]))
        loss = loss_res + loss_bc + 10 * loss_ic
        optim.zero_grad()
        loss.backward()
        return loss
...
save_dir = '/kaggle/working/reaction'
os.makedirs(save_dir, exist_ok=True) # exist_ok=True 表示如果文件夹已经存在，就直接跳过，不会报错
torch.save(model.state_dict(), f'/kaggle/working/reaction/1dreaction_{args.model}_point.pt')
...
plt.tight_layout()
#plt.axis('off')
plt.savefig(f'/kaggle/working/reaction/1d_reaction_{args.model}_{num_step}_{step_size}_pred.pdf', bbox_inches='tight')
...
plt.tight_layout()
#plt.axis('off')
plt.savefig('/kaggle/working/reaction/1d_reaction_exact.pdf', bbox_inches='tight')
...
plt.tight_layout()
#plt.axis('off')
plt.savefig(f'/kaggle/working/reaction/1d_reaction_{args.model}_{num_step}_{step_size}_error.pdf', bbox_inches='tight')
lines_to_save = [
    f'Loss Res: {loss_track[-1][0]:4f}, Loss_BC: {loss_track[-1][1]:4f}, Loss_IC: {loss_track[-1][2]:4f}',
...
save_path = f'/kaggle/working/reaction/metrics_{args.model}_{num_step}.txt'
# 3. 执行写入操作
