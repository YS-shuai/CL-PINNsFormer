...
os.makedirs('./results/wave/', exist_ok=True)
checkpoint_path = f'./results/wave/checkpoint_wave_{args.model}_{num_step}_{step_size}.pt'
...
if os.path.exists(checkpoint_path):
    print(f"检测到断点，正在从 {checkpoint_path} 恢复训练...")
    # map_location 确保在不同设备（CPU/GPU）间切换时的兼容性
    ...
    print(f"恢复成功！从第 {start_epoch} 轮继续。")
else:
    print("未发现检查点，开始全新训练。")
...
'''  
    if i % 20 == 0:
        ...
        w3 = K.item() / K3.item()
'''
for i in tqdm(range(start_epoch, 1000)):
    def closure():
        ...
        u_x = torch.autograd.grad(pred_res, x_res, grad_outputs=torch.ones_like(pred_res), retain_graph=True,
                                  create_graph=True)[0]
        u_xx = \
        torch.autograd.grad(u_x, x_res, grad_outputs=torch.ones_like(pred_res), retain_graph=True, create_graph=True)[0]
        u_t = torch.autograd.grad(pred_res, t_res, grad_outputs=torch.ones_like(pred_res), retain_graph=True,
                                  create_graph=True)[0]
        u_tt = \
        torch.autograd.grad(u_t, t_res, grad_outputs=torch.ones_like(pred_res), retain_graph=True, create_graph=True)[0]
    ...
    if i % 20 == 0:
        print('Loss Res: {:4f}, Loss_IC1: {:4f} ,Loss_IC2: {:4f}, Loss_BC: {:4f}, Loss_SQ: {:4f}'.format(
            loss_track[-1][0], loss_track[-1][1], loss_track[-1][2], loss_track[-1][3], loss_track[-1][4]))
    if (i + 1) % 50 == 0:
        torch.save({
            'epoch': i,
...
save_path = f'./results/wave/metric_{args.model}_{num_step}.txt'
