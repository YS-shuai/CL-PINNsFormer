...
for i in tqdm(range(500)):
    def closure():
        ...
        if i % 20 == 0:
            print('Loss Res: {:4f}, Loss_BC: {:4f}, Loss_IC: {:4f}, Loss_SQ: {:4f}'.format(
        ...
        loss = loss_res + loss_bc + 10 * loss_ic + 1000 * loss_seq
        # loss = loss_ic
        optim.zero_grad()
        loss.backward()
        return loss
...
if not os.path.exists('./results/reaction/'):
    os.makedirs('./results/reaction/')

torch.save(model.state_dict(), f'./results/reaction/1dwave_{args.model}_{num_step}_{step_size}.pt')
...
plt.tight_layout()
# plt.axis('off')
plt.savefig(f'./results/reaction/1d_reaction_{args.model}_{num_step}_{step_size}_pred.pdf', bbox_inches='tight')
...
plt.tight_layout()
# plt.axis('off')
plt.savefig('./results/reaction/1d_reaction_exact.pdf', bbox_inches='tight')
...
plt.tight_layout()
# plt.axis('off')
plt.savefig(f'./results/reaction/1d_reaction_{args.model}_{num_step}_{step_size}_error.pdf', bbox_inches='tight')
...
save_path = f'./results/reaction/metrics_{args.model}_{num_step}.txt'
